...
ContextMint Bridge generates a long-term Ed25519 signing keypair and an X25519 key-exchange keypair the first time it starts. These keys are used to authenticate the extension to MCP servers. They are stored in the extension's own IndexedDB, which websites and the extension's content scripts cannot access, and the private keys are held as non-extractable keys: the browser can sign with them, but will not hand their bytes to anyone, including the extension itself. Some browsers' storage cannot keep a non-extractable key-exchange key (Safari, today); there the extension stores that one key encrypted under a separate non-extractable key held in the same storage, and only if the browser cannot keep even that key does it store the key-exchange key as plain key bytes in the extension's own storage — still out of reach of websites and content scripts. The extension decides this by testing what the storage can hold, not by guessing from the browser's name. (Earlier versions kept them in `chrome.storage.local`; the first start after upgrading moves them and deletes the old copy.)
