...
| `npm run typecheck`                                          | `tsc -b packages/extension-core` (source), then `tsc -p tsconfig.tests.json` (every test file, plus the build scripts — extension-chrome's `build.ts` and `build-lib.ts`, extension-safari's `build.ts` and `manifest.ts` — and `vitest.config.ts`). vitest does not typecheck, so CI runs this before `npm test`. extension-chrome's source is typechecked by its esbuild build. |
